...
const adminAuditExportRateLimiter = createRateLimiter({
  windowMs: 60 * 60 * 1000,
  maxRequests: 20,
  keyPrefix: "api:admin:csv:audit:export",
});
...
export async function GET(request: Request) {
  ...
  const rateLimitResult = await adminAuditExportRateLimiter.limit(`admin:csv:audit:export:${adminUserId}`);
  if (!rateLimitResult.success) {
    const retryAfterSeconds = Math.max(1, Math.ceil((rateLimitResult.reset - Date.now()) / 1000));
    return Response.json(
      { error: "Too many audit export requests. Please try again later." },
      {
        status: 429,
        headers: {
